      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Potrošnja i korišćenje LLM-a za generisanje dokumenata
          </p>
        </div>
            ...
            <SelectContent>
              ...
              {documentTypes.map((t) => (
                <SelectItem
                  ...
                  value={String(t.document_type_id)}
                >
                  {t.name}
                </SelectItem>
              ))}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm py-20 text-center">
          ...
          <p className="text-slate-600 font-medium">
            Još nema podataka o potrošnji
          </p>
          <p className="text-slate-400 text-sm mt-1">
            Pokreni generisanje ili doradu dokumenta da bi se ovde pojavila
            analitika.
          </p>
        </div>
      ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard
                icon={Activity}
                label="Generisanja"
                value={fmtInt(s.total_generations)}
              ...
              <StatCard
                ...
                value={fmtCost(s.estimated_cost)}
                accent="bg-amber-50 text-amber-600"
              />
              <StatCard
                icon={Timer}
                label="Prosečna latencija"
                value={`${fmtInt(Math.round(s.avg_latency_ms))} ms`}
                  ...
                  {data.recent.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${r.generation_type === "refine" ? "bg-violet-50 text-violet-700 border-violet-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}
                        >
                          {r.generation_type}
                        </span>
                      </td>
            ...
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Snapshot izveštaji generisani PL/pgSQL procedurom za izabrani
                  period
                </p>
              </div>
              <Button
                ...
                disabled={generating}
                className="bg-slate-800 hover:bg-slate-700 text-white gap-1.5 h-9"
              >
                <Plus size={15} />{" "}
                {generating ? "Generišem..." : "Generiši izveštaj"}
              </Button>
            </div>
            {reports.length === 0 ? (
              <div className="px-5 py-12 text-center">
                ...
                <p className="text-slate-500 text-sm">
                  Još nema sačuvanih izveštaja.
                </p>
                <p className="text-slate-400 text-xs mt-1">
                  Klikni „Generiši izveštaj" za snapshot izabranog perioda.
                </p>
              </div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {reports.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-right text-slate-700 tabular-nums">
                        {fmtInt(r.total_generations)}
                      </td>
                      <td className="px-5 py-3 text-right text-slate-700 tabular-nums">
                        {fmtInt(r.total_tokens)}
                      </td>
                      <td className="px-5 py-3 text-right text-slate-700 tabular-nums">
                        {r.total_cost != null ? fmtCost(r.total_cost) : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {selectedReport && (
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="rounded-lg bg-slate-50 p-3">
                      <div className="text-lg font-bold text-slate-900 tabular-nums">
                        {fmtInt(selectedReport.total_generations)}
                      </div>
                      <div className="text-xs text-slate-500">Generisanja</div>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                      <div className="text-lg font-bold text-slate-900 tabular-nums">
                        {fmtInt(selectedReport.total_tokens)}
                      </div>
                      <div className="text-xs text-slate-500">Tokeni</div>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                      <div className="text-lg font-bold text-slate-900 tabular-nums">
                        {selectedReport.total_cost != null
                          ? fmtCost(selectedReport.total_cost)
                          : "—"}
                      </div>
                  ...
                  <div className="text-xs text-slate-500">
                    Period: {fmtPeriod(selectedReport)}
                  </div>
                        ...
                        {(selectedReport.items || []).map((it, i) => (
                          <tr key={i}>
                            ...
                            <td className="px-4 py-2 text-right text-slate-600 tabular-nums">
                              {fmtInt(it.generations)}
                            </td>
                            <td className="px-4 py-2 text-right text-slate-600 tabular-nums">
                              {fmtInt(it.total_tokens)}
                            </td>
                            <td className="px-4 py-2 text-right text-slate-600 tabular-nums">
                              {it.estimated_cost != null
                                ? fmtCost(it.estimated_cost)
                                : "—"}
                            </td>
                          </tr>
                        ))}
