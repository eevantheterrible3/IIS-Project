...
class DocumentActivityReportService:
    ...
    async def generate_report(self, document_id: str) -> BytesIO | None:
        ...
        def get_activity_text(activity_type):
            value = activity_type.value if hasattr(activity_type, "value") else str(activity_type)
        ...
        def draw_table_header():
            ...
            draw_text("Activity", margin + 190, y, "Helvetica-Bold", 10, colors.HexColor("#111827"))
            draw_text("Date", margin + 360, y, "Helvetica-Bold", 10, colors.HexColor("#111827"))
        ...
        pdf.rect(margin, y - 78, width - 2 * margin, 90, fill=True, stroke=False)

        pdf.setStrokeColor(colors.HexColor("#e5e7eb"))
        pdf.rect(margin, y - 78, width - 2 * margin, 90, fill=False, stroke=True)
        ...
        draw_text(shorten(document.name, 65), margin + 90, y - 5)

        draw_text("Project:", margin + 15, y - 25, "Helvetica-Bold", 10, colors.HexColor("#374151"))
        draw_text(shorten(project_name, 65), margin + 90, y - 25)

        draw_text("Document ID:", margin + 15, y - 45, "Helvetica-Bold", 10, colors.HexColor("#374151"))
        draw_text(document.document_id, margin + 90, y - 45, "Helvetica", 9, colors.HexColor("#6b7280"))
        ...
        if not activities:
            ...
            for activity in activities:
                ...
                user_name = shorten(get_user_name(activity), 28)
                activity_text = shorten(get_activity_text(activity.type), 28)
                ...
                draw_text(activity_text, margin + 190, y, "Helvetica", 10, colors.HexColor("#374151"))
                draw_text(activity_date, margin + 360, y, "Helvetica", 10, colors.HexColor("#6b7280"))
